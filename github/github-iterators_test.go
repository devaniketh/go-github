...
func TestBillingService_ListOrganizationBudgetsIter(t *testing.T) {
	t.Parallel()
	client, mux, _ := setup(t)
	var callNum int
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
	...
	iter := client.Billing.ListOrganizationBudgetsIter(t.Context(), "", nil)
	var gotItems int
	for _, err := range iter {
		gotItems++
		if err != nil {
	...
	if want := 7; gotItems != want {
		t.Errorf("client.Billing.ListOrganizationBudgetsIter call 1 got %v items; want %v", gotItems, want)
	}

	opts := &OrganizationListBudgetsOptions{}
	iter = client.Billing.ListOrganizationBudgetsIter(t.Context(), "", opts)
	gotItems = 0
	for _, err := range iter {
		gotItems++
		if err != nil {
	...
	if want := 2; gotItems != want {
		t.Errorf("client.Billing.ListOrganizationBudgetsIter call 2 got %v items; want %v", gotItems, want)
	}
	...
	if gotItems != 1 {
		t.Errorf("client.Billing.ListOrganizationBudgetsIter call 3 got %v items; want 1 (an error)", gotItems)
	}

	iter = client.Billing.ListOrganizationBudgetsIter(t.Context(), "", nil)
	gotItems = 0
	iter(func(item *OrganizationBudget, err error) bool {
		gotItems++
		if err != nil {
			t.Errorf("Unexpected error: %v", err)
		}
	...
	if gotItems != 1 {
		t.Errorf("client.Billing.ListOrganizationBudgetsIter call 4 got %v items; want 1 (an error)", gotItems)
	}
}
